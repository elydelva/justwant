...
async function applyEntryOptions(
  ...
  if (e.opts?.ttl) {
    ...
    if (parsed !== undefined) {
      ...
      if (ttlSec > 0) await redis.expire(prefix(e.key, keyPrefix ?? ""), ttlSec);
    }
  }
...
export function redisAdapter(options: RedisAdapterOptions): CacheAdapter {
  ...
  return {
    ...
    async delete(key: string): Promise<void> {
      const k = prefix(key, keyPrefix);
      await redis.del(k);
      if (redis.smembers && redis.srem) {
        // Cannot easily remove from tag sets without scanning; leave orphaned refs
        // They will be cleaned when tag is invalidated
      }
    },
